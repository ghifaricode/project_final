    ...
    <div class="w-full mb-1">
        ...
        <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
            <div class="flex items-center mb-4 sm:mb-0">
                <form class="sm:pr-3 w-full" action="user" method="GET">
...
<div id="drawer-create-article"
    ...
    <h5 id="drawer-label"
        class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-[#AEB9E1]">New User</h5>
        ...
        <div class="space-y-4 text-left">
            <div class="pb-4">
                ...
                <input type="text" name="name" id="name" required
                    ...
                    value="" placeholder="Type title of article..." required="">
            </div>
            <div class="pb-4">
            ...
            <div class="pb-4">
                ...
                <input type="text" name="phone" id="phone" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primaryEx-600 focus:border-primaryEx-600 block w-full p-2.5 dark:bg-[#0B1739] dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primaryEx-500 dark:focus:border-primaryEx-500"
                    value="" placeholder="08xxxxxxxxxx" required="">
            </div>
            <div class="pb-4">
            ...
            <div class="pb-4">
                ...
                <select name="role_id" id="role_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primaryEx-600 focus:border-primaryEx-600 block w-full p-2.5 dark:bg-[#0B1739] dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primaryEx-500 dark:focus:border-primaryEx-500" required>
                ...
                <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $user->name }}</td>
                            <td class="h-fit p-4 overflow-hidden text-base font-medium text-gray-500 truncate xl:max-w-xs dark:text-gray-400">
                                <a href="mailto:{{ $user->email }}" target="_blank" class="hover:text-primaryEx-600 hover:dark:text-primaryEx-500 hover:underline">
                                    {{ $user->email }}
                                </a>
                            </td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $user->phone }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ substr($user->password, 0, 10) }}...</td>
                            <td class="h-fit p-4 capitalize text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $user->role_id == 1 ? 'Admin' : 'User' }}
                            </td>
                            <td class="h-fit p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                <div class="text-base font-semibold text-gray-900 dark:text-white">{{ date('d F Y', strtotime($user->created_at)) }}</div>
                            </td>
                            <td class="p-4 space-x-2 whitespace-nowrap text-center">
                                {{-- Update button --}}
                                <button type="button" id="updateProductButton-{{ $user->id }}"
                                    data-drawer-target="drawer-update-{{ $user->id }}"
                                    data-drawer-show="drawer-update-{{ $user->id }}"
                                    aria-controls="drawer-update-{{ $user->id }}"
                                    data-drawer-placement="right"
                                    onclick="showUpdateModal('{{ $user->id }}')"
                                ...
                                <div id="drawer-update-{{ $user->id }}"
                                    ...
                                    <h5 id="drawer-label"
                                        ...
                                        Update User</h5>
                                    <button type="button"
                                        onclick="closeUpdateDrawer('{{ $user->id }}')"
                                        data-drawer-dismiss="drawer-update-{{ $user->id }}"
                                        aria-controls="drawer-update-{{ $user->id }}"
                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                ...
                                <button type="button" id="deleteProductButton"
                                    data-drawer-target="drawer-delete-{{ $user->id }}"
                                    data-drawer-show="drawer-delete-{{ $user->id }}"
                                    aria-controls="drawer-delete-{{ $user->id }}"
                                    data-drawer-placement="right"
                                    onclick="showDeleteDrawer('{{ $user->id }}')"
                                ...
                                <div id="drawer-delete-{{ $user->id }}"
                                    ...
                                    <h5 id="drawer-label"
                                        ...
                                        Delete item</h5>
                                    <button type="button"
                                        onclick="closeDeleteDrawer('{{ $user->id }}')"
                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                        <svg aria-hidden="true" class="w-5 h-5" fill="currentColor"
                                            viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd"
...
<div class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
    <div class="flex items-center mb-4 sm:mb-0">
        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Showing <span class="font-semibold text-gray-900 dark:text-white">{{ $users->count() }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ $users->total() }}</span></span>
    </div>
    <div class="flex items-center space-x-3">
        <a href="{{ ($users->currentPage() == 1) ? $users->url(1) : $users->previousPageUrl() }}" class="inline-flex items-center justify-center flex-1 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primaryEx-700 hover:bg-primaryEx-800 focus:ring-4 focus:ring-primaryEx-300 dark:bg-primaryEx-600 dark:hover:bg-primaryEx-700 dark:focus:ring-primaryEx-800">
            <svg class="w-5 h-5 mr-1 -ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"></path>
            </svg>
            Previous
        </a>
        <a href="{{ ($users->currentPage() == $users->lastPage()) ? $users->url($users->lastPage()) : $users->nextPageUrl() }}" class="inline-flex items-center justify-center flex-1 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primaryEx-700 hover:bg-primaryEx-800 focus:ring-4 focus:ring-primaryEx-300 dark:bg-primaryEx-600 dark:hover:bg-primaryEx-700 dark:focus:ring-primaryEx-800">
            Next
...
<script>
    ...
    function showDeleteDrawer(userId) {
        document.getElementById('drawer-delete-' + userId).classList.remove('translate-x-full');
    }

    function closeDeleteDrawer(userId) {
        document.getElementById('drawer-delete-' + userId).classList.add('translate-x-full');
    }
    ...
    function closeDrawer() {
        document.getElementById('drawer-create-article').classList.add('translate-x-full');
    }

    document.getElementById('createProductButton').addEventListener('click', function() {
        document.getElementById('drawer-create-article').classList.toggle('translate-x-full');
    });
    ...
    function showUpdateModal(userId) {
        const modal = document.getElementById(`drawer-update-${userId}`);
        if (modal) {
    ...
    function closeUpdateDrawer(userId) {
        const drawer = document.getElementById(`drawer-update-${userId}`);
        if (drawer) {
