    ...
    <div class="w-full mb-1">
        ...
        <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
            <div class="flex items-center mb-4 sm:mb-0">
                <form class="sm:pr-3 w-full" action="reservations" method="GET">
...
<div id="drawer-create-article"
    ...
    <form action="{{ route('admin.reservation.store') }}" method="POST">
        ...
        <div class="space-y-4 text-left">
            ...
            <div class="pb-4">
                <label for="total_guests" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Total Guests</label>
                <input type="number" name="total_guests" id="total_guests" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primaryEx-600 focus:border-primaryEx-600 block w-full p-2.5 dark:bg-[#0B1739] dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primaryEx-500 dark:focus:border-primaryEx-500"
                ...
                <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $reservation->user->name }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $reservation->room->name }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $reservation->check_in }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $reservation->check_out }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $reservation->total_guests }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $reservation->total_price }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $reservation->status }}</td>
                            <td class="h-fit p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                <div class="text-base font-semibold text-gray-900 dark:text-white">{{ date('d F Y', strtotime($reservation->created_at)) }}</div>
                            </td>
                            <td class="p-4 space-x-2 whitespace-nowrap text-center">
                                {{-- Update button --}}
                                <button type="button" id="updateProductButton-{{ $reservation->id }}"
                                    data-drawer-target="drawer-update-{{ $reservation->id }}"
                                    data-drawer-show="drawer-update-{{ $reservation->id }}"
                                    aria-controls="drawer-update-{{ $reservation->id }}"
                                    data-drawer-placement="right"
                                    onclick="showUpdateModal('{{ $reservation->id }}')"
                                ...
                                {{-- Update modal --}}
                                <div id="drawer-update-{{ $reservation->id }}"
                                    ...
                                    <h5 id="drawer-label"
                                        ...
                                        Update Reservation</h5>
                                    <button type="button"
                                        onclick="closeUpdateDrawer('{{ $reservation->id }}')"
                                        data-drawer-dismiss="drawer-update-{{ $reservation->id }}"
                                        aria-controls="drawer-update-{{ $reservation->id }}"
                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                    ...
                                    <form action="reservation/{{ $reservation->id }}" method="POST">
                                        ...
                                        <div class="space-y-4 text-left">
                                            ...
                                            <div class="pb-4">
                                                ...
                                                <input type="date" name="check_in" id="check_in" required
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primaryEx-600 focus:border-primaryEx-600 block w-full p-2.5 dark:bg-[#0B1739] dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primaryEx-500 dark:focus:border-primaryEx-500"
                                                    value="{{ date('Y-m-d', strtotime($reservation->check_in)) }}">
                                            </div>
                                            <div class="pb-4">
                                                <label for="check_out"
                                                ...
                                                <input type="date" name="check_out" id="check_out" required
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primaryEx-600 focus:border-primaryEx-600 block w-full p-2.5 dark:bg-[#0B1739] dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primaryEx-500 dark:focus:border-primaryEx-500"
                                                    value="{{ date('Y-m-d', strtotime($reservation->check_out)) }}">
                                            </div>
                                            <div class="pb-4">
                                                <label for="total_guests"
                                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Total Guests</label>
                                                <input type="number" name="total_guests" id="total_guests" required
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primaryEx-600 focus:border-primaryEx-600 block w-full p-2.5 dark:bg-[#0B1739] dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primaryEx-500 dark:focus:border-primaryEx-500"
                                        ...
                                        <div
                                            ...
                                            <button type="submit"
                                                class="w-full justify-center text-white bg-primaryEx-700 hover:bg-primaryEx-800 focus:ring-4 focus:outline-none focus:ring-primaryEx-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primaryEx-600 dark:hover:bg-primaryEx-700 dark:focus:ring-primaryEx-800">
                                                Update
                                            </button>
                                            <button type="button"
                                                onclick="closeUpdateDrawer('{{ $reservation->id }}')"
                                ...
                                <button type="button" id="deleteProductButton"
                                    data-drawer-target="drawer-delete-{{ $reservation->id }}"
                                    data-drawer-show="drawer-delete-{{ $reservation->id }}"
                                    aria-controls="drawer-delete-{{ $reservation->id }}"
                                    data-drawer-placement="right"
                                    onclick="showDeleteDrawer('{{ $reservation->id }}')"
                                ...
                                {{-- Delete modal --}}
                                <form action="reservation/{{ $reservation->id }}" method="post"
                                    id="delete-form-{{ $reservation->id }}" class="hidden">
                                    @method('delete')
                                    @csrf
                                </form>
                                <div id="drawer-delete-{{ $reservation->id }}"
                                    ...
                                    <h5 id="drawer-label"
                                        ...
                                        Delete item</h5>
                                    <button type="button"
                                        onclick="closeDeleteDrawer('{{ $reservation->id }}')"
                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                        <svg aria-hidden="true" class="w-5 h-5" fill="currentColor"
                                            viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd"
...
<div class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
    <div class="flex items-center mb-4 sm:mb-0">
        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Showing <span class="font-semibold text-gray-900 dark:text-white">{{ $reservations->count() }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ $reservations->total() }}</span></span>
    </div>
    <div class="flex items-center space-x-3">
        <a href="{{ ($reservations->currentPage() == 1) ? $reservations->url(1) : $reservations->previousPageUrl() }}" class="inline-flex items-center justify-center flex-1 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primaryEx-700 hover:bg-primaryEx-800 focus:ring-4 focus:ring-primaryEx-300 dark:bg-primaryEx-600 dark:hover:bg-primaryEx-700 dark:focus:ring-primaryEx-800">
            <svg class="w-5 h-5 mr-1 -ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"></path>
            </svg>
            Previous
        </a>
        <a href="{{ ($reservations->currentPage() == $reservations->lastPage()) ? $reservations->url($reservations->lastPage()) : $reservations->nextPageUrl() }}" class="inline-flex items-center justify-center flex-1 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primaryEx-700 hover:bg-primaryEx-800 focus:ring-4 focus:ring-primaryEx-300 dark:bg-primaryEx-600 dark:hover:bg-primaryEx-700 dark:focus:ring-primaryEx-800">
            Next
