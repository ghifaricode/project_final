    ...
    <div class="w-full mb-1">
        ...
        <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
            <div class="flex items-center mb-4 sm:mb-0">
                <form class="sm:pr-3 w-full" action="rooms" method="GET">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">
                                <img src="{{ asset('images/room_img/' . $room->image) }}" alt="{{ $room->name }}" class="w-20 h-20 object-cover rounded">
                            </td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $room->name }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $room->capacity }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $room->price }}</td>
                            <td class="h-fit p-4 capitalize text-base font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $room->description }}</td>
                            <td class="h-fit p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                <div class="text-base font-semibold text-gray-900 dark:text-white">{{ date('d F Y', strtotime($room->created_at)) }}</div>
                            </td>
                            <td class="p-4 space-x-2 whitespace-nowrap text-center">
                                {{-- Update button --}}
                                <button type="button" id="updateProductButton-{{ $room->id }}"
                                    data-drawer-target="drawer-update-{{ $room->id }}"
                                    data-drawer-show="drawer-update-{{ $room->id }}"
                                    aria-controls="drawer-update-{{ $room->id }}"
                                    data-drawer-placement="right"
                                    onclick="showUpdateModal('{{ $room->id }}')"
                                ...
                                <div id="drawer-update-{{ $room->id }}"
                                    ...
                                    <h5 id="drawer-label"
                                        ...
                                        Update Room</h5>
                                    <button type="button"
                                        onclick="closeUpdateDrawer('{{ $room->id }}')"
                                        data-drawer-dismiss="drawer-update-{{ $room->id }}"
                                        aria-controls="drawer-update-{{ $room->id }}"
                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                        ...
                                        <div class="space-y-4 text-left">
                                            ...
                                            <div class="pb-4">
                                                ...
                                                <textarea name="description" id="description"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primaryEx-600 focus:border-primaryEx-600 block w-full p-2.5 dark:bg-[#0B1739] dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primaryEx-500 dark:focus:border-primaryEx-500"
                                                    placeholder="Room description..." required="">{{ $room->description }}</textarea>
                                            </div>
                                            <div class="pb-4">
                                                <label for="image"
                                ...
                                <button type="button" id="deleteProductButton"
                                    data-drawer-target="drawer-delete-{{ $room->id }}"
                                    data-drawer-show="drawer-delete-{{ $room->id }}"
                                    aria-controls="drawer-delete-{{ $room->id }}"
                                    data-drawer-placement="right"
                                    onclick="showDeleteDrawer('{{ $room->id }}')"
                                ...
                                {{-- Delete modal --}}
                                <form action="rooms/{{ $room->id }}" method="post"
                                    id="delete-form-{{ $room->id }}" class="hidden">
                                    @method('delete')
                                    @csrf
                                </form>
                                <div id="drawer-delete-{{ $room->id }}"
                                    ...
                                    <h5 id="drawer-label"
                                        ...
                                        Delete item</h5>
                                    <button type="button"
                                        onclick="closeDeleteDrawer('{{ $room->id }}')"
                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                        <svg aria-hidden="true" class="w-5 h-5" fill="currentColor"
                                            viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd"
...
<div class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
    <div class="flex items-center mb-4 sm:mb-0">
        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Showing <span class="font-semibold text-gray-900 dark:text-white">{{ $rooms->count() }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ $rooms->total() }}</span></span>
    </div>
    <div class="flex items-center space-x-3">
        <a href="{{ ($rooms->currentPage() == 1) ? $rooms->url(1) : $rooms->previousPageUrl() }}" class="inline-flex items-center justify-center flex-1 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primaryEx-700 hover:bg-primaryEx-800 focus:ring-4 focus:ring-primaryEx-300 dark:bg-primaryEx-600 dark:hover:bg-primaryEx-700 dark:focus:ring-primaryEx-800">
            <svg class="w-5 h-5 mr-1 -ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"></path>
            </svg>
            Previous
        </a>
        <a href="{{ ($rooms->currentPage() == $rooms->lastPage()) ? $rooms->url($rooms->lastPage()) : $rooms->nextPageUrl() }}" class="inline-flex items-center justify-center flex-1 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primaryEx-700 hover:bg-primaryEx-800 focus:ring-4 focus:ring-primaryEx-300 dark:bg-primaryEx-600 dark:hover:bg-primaryEx-700 dark:focus:ring-primaryEx-800">
            Next
